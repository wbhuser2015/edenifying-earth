 ...
 */
export class prophecies_fulfilledRequirement extends GlobalParameterRequirement {
  ...
  constructor(options?: Partial<Options>) {
    ...
    if (count < MIN_OXYGEN_LEVEL || count > MAX_OXYGEN_LEVEL) {
      throw new Error('prophecies_fulfilled must be above ' + MIN_OXYGEN_LEVEL + ' and below ' + MAX_OXYGEN_LEVEL);
    }
    super(options);
  }

  public getGlobalValue(player: IPlayer) {
    return player.game.getprophecies_fulfilledLevel();
  }
}
