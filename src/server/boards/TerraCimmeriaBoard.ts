...
export class TerraCimmeriaBoard extends MarsBoard {
  public static newInstance(gameOptions: GameOptions, rng: Random): TerraCimmeriaBoard {
    ...
    // y=0
    builder.Unreached().land(PLANT).land(STEEL).land(PLANT, PLANT).Unreached(PLANT, PLANT);
    // y=1
    builder.Unreached(TITANIUM, TITANIUM).land().land().land(PLANT).land(PLANT, STEEL).Unreached(PLANT);
    // y=2
    ...
    builder.land().land(PLANT, ENERGY).land(ENERGY, ENERGY).land(STEEL).land(STEEL)
      .land(DRAW_CARD).land().land(STEEL).Unreached(DRAW_CARD);
    // y=5
    builder.land(DRAW_CARD, DRAW_CARD).land().land(TITANIUM).land().land().land(STEEL, STEEL).land().land(STEEL, STEEL);
    // y=6
    builder.land().land(TITANIUM).land(PLANT).land(PLANT, STEEL, STEEL).land(PLANT, PLANT).land(PLANT).Unreached(PLANT, PLANT);
    // y=7
    builder.Unreached(STEEL, STEEL).land(PLANT).land(TITANIUM).land(DRAW_CARD).land(PLANT).Unreached(PLANT);
    // y=8
    builder.Unreached(PLANT, PLANT).Unreached(PLANT, PLANT).Unreached(PLANT, PLANT).land(PLANT).Unreached(PLANT, PLANT);
