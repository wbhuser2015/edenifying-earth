...
const baseManifestPath = path.join(__dirname, `src/server/cards/StandardCardManifest.ts`);
...
// **5. Global Search & Remove References in the Entire Project**
const searchAndRemoveReferences = (dir) => {
  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      searchAndRemoveReferences(filePath); // Recursively search subdirectories
    } else if (file.endsWith('.ts') || file.endsWith('.vue')) {
      let fileContent = fs.readFileSync(filePath, 'utf8');

      // Remove any reference to the card
      const referenceRegex = new RegExp(`\\b(CardName|TileType|Tile)\\.${CARD_CONSTANT}\\b`, 'g');
      if (referenceRegex.test(fileContent)) {
        fileContent = fileContent.replace(referenceRegex, '');
        fs.writeFileSync(filePath, fileContent);
        console.log(`✅ Removed references to ${CARD_CONSTANT} from ${filePath}`);
      }
    }
  });
};

// Run the global search function
console.log("🔍 Searching and removing references across entire codebase...");
searchAndRemoveReferences(srcPath);
...
// **6. Build the Project to Ensure Removal**
console.log("🚀 Running TypeScript build...");
require('child_process').execSync('npm run build', { stdio: 'inherit' });

console.log(`🎉 Card '${cardName}' successfully removed from all references!`);
